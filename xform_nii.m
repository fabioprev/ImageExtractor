...
%
function nii = xform_nii(nii, tolerance, preferredForm, filename)
   ...
   try
      [hdr,orient]=change_hdr(hdr,tolerance,preferredForm);
	   ...
	   res = strfind(Exception.message,'Non-orthogonal rotation');
	   
	   if (size(res,1) > 0)
		   niiNew = reslice_nii(filename,''); % The new nii file will be returned instead of being saved.
